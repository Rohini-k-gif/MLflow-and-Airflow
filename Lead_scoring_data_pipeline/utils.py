...
def load_data_into_db(db_path, db_file_name,data_directory):
    ...
    leadscoring = load_data( [f"{data_directory}leadscoring_inference.csv",])[0]
    leadscoring.reset_index(drop=True)
    leadscoring.to_sql(name='loaded_data', con=cnx, if_exists='replace')
    cnx.close()
    return "Writing to DataBase loaded_data Done or Data Already available in Table. Check Logs."
